...
const sanitizeUrl = url => {
  ...
  searchParams
    .keys()
    .forEach(searchParamsKey => searchParams.set(searchParamsKey, '*'));
  newUrl.search = searchParams.toString();
  return newUrl.toString();
};

if (!isNullOrUndefined(process.env.LOGROCKET_ENV)) {
  ...
  LogRocket.init(process.env.LOGROCKET_ENV, {
    ...
    network: {
      requestSanitizer: request => {
        const modRequest = requestSanitizer(request);
      ...
      responseSanitizer: response => {
        const modResponse = responseSanitizer(response);
